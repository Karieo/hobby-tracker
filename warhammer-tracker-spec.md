...
| §7 | **Shortfalls → purchases** — invert `kit_templates`, show the overage, and always show the à la carte total beside the bundle total | **Built,** 2026-08-26. `shopping.py` and `/shopping`, computed live like the gap report and the backlog. The wishlist answers in datasheets and a shop sells boxes, so this covers the shortfall greedily out of `kit_templates` — most models still missing first, ties to the smaller overage — and buys one box at a time, so twenty Boyz against a box of ten is that box twice rather than any reasoning about multiples. The overage is carried per line and totalled, because four boxes with forty spare is not obviously better than five with six. Bundle against à la carte is the same cover run twice, once over every box and once over only the single-unit ones: two totals from one mechanism, so the comparison cannot be computed differently from the thing it compares to. Prices are three-state for the same reason list validation is — most templates carry no `rrp_cents`, a total that silently skipped them would read *low*, and low is the one direction a shopping total must never be wrong in. So *priced* is a figure, *partial* is a floor shown as "at least", and *unpriced* shows none. No saving is claimed unless both sides are fully priced and both cover the ground; a negative one is reported as it stands, since a comparison that only ever flatters the bundle is not a comparison. Anything no box contains is named rather than dropped. |
